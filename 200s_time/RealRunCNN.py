...
def plot_data_histogram(prob_station, prob_confirmed=[], if_confirmed=False):
...
def plot_sim_histogram(prob_RCR, prob_Backlobe, ampforsim):
    ...
    dense_val = False
    fig, ax = plt.subplots(figsize=(8, 6))  
    hist_values, bin_edges, _ = ax.hist(prob_Backlobe, bins=20, range=(0, 1), histtype='step', color='red', linestyle='solid', label=f'Backlobe {len(prob_Backlobe)}', density=dense_val)
    ax.hist(prob_RCR, bins=20, range=(0, 1), histtype='step', color='blue', linestyle='solid', label=f'RCR {len(prob_RCR)}', density=dense_val)
...
if __name__ == '__main__':
    ...
    if pickRundata == 'sim':
        ...
        plot_data_histogram(prob_station)
