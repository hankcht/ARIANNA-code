...
if __name__ == "__main__":
    ...
    for index in indices:
        ...
        print(f'number of sim is{len(sim_RCR)}')
        pT(sim_RCR[index], 'test plot new sim', f'/pub/tangch3/ARIANNA/DeepLearning/test_new_sim_noisefalse_{amp}_{index}.png')


        sim_RCR = load_sim_rcr(sim_folder, noise_enabled=True, filter_enabled=False, amp=amp)
        print(f'number of sim is{len(sim_RCR)}')
        pT(sim_RCR[index], 'test plot new sim', f'/pub/tangch3/ARIANNA/DeepLearning/test_new_sim_filterfalse_{amp}_{index}.png')
