...
#this file:
#   [1] finds the indices of selected events,
...
if __name__ == "__main__":
    ...
    check_SNR = np.load(f'/pub/tangch3/ARIANNA/DeepLearning/circled_events/Station_SNR/{amp_type}/Stn{station_id}_SNR.npy')
    check_Chi = np.load(f'/pub/tangch3/ARIANNA/DeepLearning/circled_events/Station_Chi/{amp_type}/Stn{station_id}_Chi.npy')
    # result = all(x == y for x, y in zip(PassingCut_SNRs, check_SNR))
    # print(check_SNR,check_Chi) 

    selected_chi = []
    selected_chi_idx = []
    for i, chi in enumerate(check_Chi):
        if chi >= 0.5:
            selected_chi.append(chi)
            selected_chi_idx.append(i)
    print(len(selected_chi))
    print(selected_chi_idx)
    print(check_Chi)

    # # result2 = all(x == y for x, y in zip(PassingCut_RCR_Chi, check_Chi))
    # # print(result2)   
    # for x, y in zip(PassingCut_RCR_Chi, check_Chi):
    #     if x != y:
    #         print(x,y)
