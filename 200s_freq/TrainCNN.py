...
# with open('/dfs8/sbarwick_lab/ariannaproject/rricesmi/numpy_arrays/simulatedRCRs/200s_2.9.24/FilteredSimRCR_200s_6312events_part0.npy', mode="rb") as f:
#     data = f.read()
#     for c in data:
#         print(c, end = " ")
...
#y is output array
y = np.vstack((np.zeros((RCR_freq.shape[0], 1)), np.ones((Backlobe_freq.shape[0], 1))))
...
def training(j):
    ...
    # file_path = f'{model_path}{timestamp_with_seconds}_RCR_Backlobe_model_2Layer_{j}_history.pkl'
    ...
    plt.savefig(f'{plots_path_loss}loss_plot_{timestamp_with_seconds}_RCR_Backlobe_model_2Layer_{j}.png')
    plt.clf()
    ...
    plt.savefig(f'{plots_path_accuracy}accuracy_plot_{timestamp_with_seconds}_RCR_Backlobe_model_2Layer_{j}.png')
    plt.clf()
    ...
    model.save(f'{model_path}{timestamp_with_seconds}_RCR_Backlobe_model_2Layer_{j}.h5')
